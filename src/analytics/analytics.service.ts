...
export class AnalyticsService implements OnModuleInit, OnModuleDestroy {
  ...
  private updateSlowestEndpoints(
    method: string,
    url: string,
    duration: number,
  ) {
  ...
  private async checkThresholds(data: MetricPoint): Promise<void> {
    ...
    if (data.cpu > 85) {
      await createAlert(
        'WARNING',
        `High CPU Usage detected: ${data.cpu}%`,
        'SYSTEM',
      );
    }
    ...
    if (data.memory > 90) {
      await createAlert(
        'WARNING',
        `High Memory Usage detected: ${data.memory.toFixed(1)}%`,
        'SYSTEM',
      );
    }
    ...
    if (data.latency > 1000) {
      await createAlert(
        'WARNING',
        `High Database Latency: ${data.latency}ms`,
        'DATABASE',
      );
    }
    ...
    if (data.dbCacheHitRatio < 95) {
      await createAlert(
        'INFO',
        `Database Cache Hit Ratio dropped to ${data.dbCacheHitRatio}%`,
        'DATABASE',
      );
    }
  }
